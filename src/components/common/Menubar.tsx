...
const Card = (props: { icon: React.JSX.Element, title: string, path: string, isOpen?: boolean, setIsOpen?: Dispatch<SetStateAction<boolean>> }) => {
    const path = usePathname();
    const maincssClass = `flex flex-row space-x-10 my-2 text-gray-500 hover:bg-[#F0F0F0] rounded-md outline-0 group ${path === props.path && props.title !== "StockInsights" ? "bg-[#F0F0F0]" : ""}`;
    const textcssClass = `opacity-0 visibility group-hover:opacity-100 group-hover:visibility-visible ${props.isOpen ? "opacity-100 visibility-visible" : ""} ${props.title === "Options" ? "font-semibold" : ""} text-sm my-[9px] text-gray-500 transition-opacity duration-200 ease-in-out  `;
    return (
        ( props.title!=="Options" ?
    ...
    return (
        <div>
            ...
            <div className='absolute h-screen w-fit left-0 top-0 flex flex-row bg-[#FCFCFC] hidden md:flex'>
                <div className={`group hover:w-44 hover:shadow-xl  ${isOpen? "w-44 shadow-xl":"w-14"} transition-all duration-200 ease-in-out px-2`}>
                    {/* Logo */}
                    ...
                    } title="StockInsights" path={`/${props.countrycode}/ai-assist`} isOpen={isOpen}/>
                    </div>
                    {/* other items */}
                    <Card icon={<Sparkles className='m-3 fixed w-4 h-4 text-[#B04425]'/>} title="AI&nbsp;Assist" path={`/${props.countrycode}/ai-assist`} isOpen={isOpen}/>
                    <Card icon={<Search className='m-3 fixed w-4 h-4'/>} title="Search&nbsp;Fillings" path={`/${props.countrycode}/filings`} isOpen={isOpen} />
                    <Card icon={<Megaphone className='m-3 fixed w-4 h-4'/>} title="Announcements" path={`/${props.countrycode}/announcements`} isOpen={isOpen}/>
                    <Card icon={<Heart className='m-3 fixed w-4 h-4'/>} title="My&nbsp;Watchlist" path={`/${props.countrycode}/watchlist`} isOpen={isOpen}/>
                    <Card icon={<Mail className='m-3 fixed w-4 h-4'/>} title="Email&nbsp;Updates" path={`/${props.countrycode}/notifications`} isOpen={isOpen}/>
                    {/* option */}
                    <div className='absolute bottom-1'>
                    <Card icon={<EllipsisVertical className='m-3 ml-[14px] fixed w-4 h-4'/>} title="Options" path='' isOpen={isOpen} setIsOpen={setIsOpen}/>
                    </div>
