...
import { Sparkles, Search, Megaphone, Heart, Mail, EllipsisVertical } from 'lucide-react';
...
const Card = (props: { icon: React.JSX.Element, title: string, path: string, }) => {
    const path = usePathname();
    return (
        <Link href={props.path} className={`flex flex-row space-x-10 my-2 text-gray-500 hover:bg-[#F0F0F0] ${path === props.path ? "bg-[#F0F0F0]" : ""} rounded-md`}>
            {props.icon} 
    ...
    return (
        <div>
            ...
            <div className="group w-14 hover:w-44 transition-all duration-200 ease-in-out hover:shadow-xl px-2">
                ...
                } title="StockInsights" path=''/>
                </div>
                <Card icon={<Sparkles className='m-3 fixed w-4 h-4 text-[#B04425]'/>} title="AI&nbsp;Assist" path={`/${props.countrycode}/ai-assist`}/>
                <Card icon={<Search className='m-3 fixed w-4 h-4'/>} title="Search&nbsp;Fillings" path={`/${props.countrycode}/filings`} />
                <Card icon={<Megaphone className='m-3 fixed w-4 h-4'/>} title="Announcements" path={`/${props.countrycode}/announcements`}/>
                <Card icon={<Heart className='m-3 fixed w-4 h-4'/>} title="My&nbsp;Watchlist" path={`/${props.countrycode}/watchlist`}/>
                <Card icon={<Mail className='m-3 fixed w-4 h-4'/>} title="Email&nbsp;Updates" path={`/${props.countrycode}/notifications`}/>

                <div className='absolute bottom-0'>
                <Card icon={<EllipsisVertical className='m-3 ml-4 fixed w-4 h-4'/>} title="Email&nbsp;Updates" path=''/>
                </div>
